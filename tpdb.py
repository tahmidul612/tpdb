...
def downloadPoster(url):
    ...
    if response.status_code == 200:
        ...
        )  # Ensure totalBytes is set
        with (
            open(os.path.join(saveDir, filename), "wb") as file,
            alive_bar(totalBytes, title=filename, force_tty=True) as bar,
        ):
...
if __name__ == "__main__":
        ...
        if input("Save config? (y/n): ") == "y":
            ...
            if not os.path.exists(config_file_path):
                ...
                if update_config(config_file_path):
                    ...
                    print(
                        "Config file already contains data, but server_baseurl and server_token were not found. Please update it manually."
                    )
    ...
    parser.add_argument(
        ...
        default=libraryNames,
        help="Name of the Plex libraries to process",
    )
    parser.add_argument(
        "--action",
        nargs="?",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Find and process unlinked posters",
    )
    parser.add_argument(
        "-f",
        "--force",
    ...
    parser.add_argument(
        "--filter", nargs="?", help="String filter for source poster folders"
    )
    parser.add_argument(
        "-a",
        "--all",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Copy posters to media folders",
    )
    parser.add_argument(
        "-d", "--download", nargs="?", help="Download a poster from a URL"
    )
    ...
    if opts.libraries:
        for library_name in opts.libraries:
            selectedLibrary = next(
                (lib for lib in allLibraries if lib.title == library_name), None
            )
            ...
            if selectedLibrary.type in ["movie", "show"]:
                ...
                if opts.filter:
                    ...
                    print(
                        f"Filtered poster folders to search in:\n{poster_data.posterFolders}"
                    )
                    ...
                    case "movie":
                        if opts.unlinked:
                            ...
                            if (
                                unlinkedFolders
                                and input(
                                    f"{len(unlinkedFolders)} unlinked folders found. Start processing them? (y/n): "
                                )
                                == "y"
                            ):
                            ...
                            for folder in poster_data.posterFolders:
                                ...
                                if posterExists and (
                                    opts.all
                                    or input('Process folder "%s"? (y/n):  ' % folder)
                                    == "y"
                                ):
